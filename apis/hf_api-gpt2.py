...
class HFAPI(API):
    ...
    def text_random_sampling(self, num_samples, prompt_counter=None, lens_dict=None):
        ...
        for prompt in tqdm(prompt_counter):
            ...
            if self.use_subcategory:                    
                ...
                elif self.variation_type == "phi4_rephrase_tone":
    ...
    def _rephrase(self, label, sequence, variation_type):
        ...
        if variation_type == "yelp_rephrase_tone":
            ...
            prompt = "Please rephrase the following sentences as an abstract for medical research paper in clear and natural English while preserving meaning, maintaining proper punctuation, and discard special characters, symbols:\n{} \n".format(sequence)
        elif variation_type == "sd_rephrase_tone":
            selected_style = ALL_SD_styles[random.randrange(
                len(ALL_SD_styles))]
            prompt = "Rephrase the following passage {}:\n{} \n".format(selected_style, sequence)
        elif variation_type == "phi4_rephrase_tone":
            selected_style = ALL_SD_styles[random.randrange(
                len(ALL_SD_styles))]
            # system_prompt = f"The following text is an abstracted self-disclosure statement. Use all provided words to infer the original meaning and rewrite it into a realistic self-disclosure passage {selected_style} that closely mimics the original intent:"
            system_prompt = f"The following text is an abstracted self-disclosure statement. Use its words to infer the original meaning and rewrite it into a realistic self-disclosure passage less than 800 words {selected_style} that closely mimics the original intent:"
            messages = [
                {"role": "system", "content": system_prompt},
        ...
        with torch.no_grad():
            for _ in range(self.length // step_size):
                ...
                if new_text.strip():
                    ...
                    input_ids = self.tokenizer(new_text, return_tensors="pt").input_ids.to(input_ids.device)
