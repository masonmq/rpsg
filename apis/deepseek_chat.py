...
DEFAULT_OPENAI_API_BASE = DeepSeek_CONFIG['DeepSeek-R1']['openai_api_base']
OPENAI_API_KEYS = DeepSeek_CONFIG['DeepSeek-R1']['openai_api_key']
...
client = ChatCompletionsClient(
    endpoint=DeepSeek_CONFIG['DeepSeek-R1']['openai_api_base'],
    credential=AzureKeyCredential(DeepSeek_CONFIG['DeepSeek-R1']['openai_api_key']),
    model = DeepSeek_CONFIG['DeepSeek-R1']['engine'],
    max_tokens=800
)

def deepseek_completions(
    ...
    **decoding_kwargs,
    ...
    if n_examples == 0:
        ...
        logging.info(
            f"Using `deepseek_completions` on {n_examples} prompts using {model_name}.")
    ...
    for completion_batch in completions:
        for completion in completion_batch:
            try:
                text_response = completion.get("text", None)
                if text_response:
                    text_response = text_response.encode('utf-8', errors='ignore').decode('utf-8')
                    completions_text.append(text_response)
                else:
                    print("\n WARNING: 'text' key missing or empty in response")
    ...
    for text in completions_text:
        # 1. Remove any <think>...</think> block (including the tags)
        text = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL)
        # 2. Remove the final explanatory sentence (assumes it starts with "This rewrite")
        if "This rewrite" in text:
            text = text.split("This rewrite")[0].strip()
        # 3. Remove surrounding quotation marks if they exist
...
def _call_chat_completion(messages, **kwargs):
    ...
    response_stream = client.complete(messages=messages, stream=True)
...
def _deepseek_completion_helper(
    ...
    **kwargs,
    ...
    for i in range(retry + 1):
        ...
        try:
            ...
            if is_chat:
                completion_response = _call_chat_completion(
                    messages=prompt_batch[0][0], **curr_kwargs)  # batch size is 1

                choices = [{"text": completion_response.get("text", " ")}]
            ...
            delay = sleep_time * (2 ** min(i, 3)) + random.uniform(0, 0.1)
            time.sleep(delay)
...
def _requires_chatml(model: str) -> bool:
    ...
    return "turbo" in model or "gpt-4" in model or 'DeepSeek' in model
