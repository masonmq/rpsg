...
def main():
    ...
    logging.info('Selection mode is fixed to rank; probabilistic selection has been removed.')
    ...
    if args.data_checkpoint_path != '':
        ...
        if args.r_data == 1:
            ...
            log_generation(fname=f'{args.result_folder}/0/real_abstracted.jsonl',
                           abstracted=np.stack([abstracted_seed_syn_samples], axis=1),
                           realdata=np.stack([seed_real_samples], axis=1))
    ...
    for t in range(start_t, len(args.num_samples_schedule)):
        ...
        for class_i, class_ in enumerate(private_classes):
            ...
            sub_new_indices = list(range(current_idx, num_samples_per_class + current_idx))
            ...
            num_generated_variation_groups = second_variants_samples_stacked.shape[1]
            new_variants_additional_info = selected_additional_info * num_generated_variation_groups
            ...
            if args.save_syn_mode == 'selected':
                ...
                all_selected_additional_info.extend(selected_additional_info)
            elif args.save_syn_mode == 'all':  # @ default
                logging.info(
                    f'save_syn_mode == all')
                all_selected_samples.extend(selected_syn_samples)
                all_selected_additional_info.extend(selected_additional_info)
        ...
        log_samples(samples=all_selected_samples, additional_info=all_selected_additional_info,
                    folder=f'{args.result_folder}/{t}')
        syn_samples = new_syn_samples  # @ syn_samples were saved in 1_all/samples.csv
        additional_info = new_additional_info
        log_samples(samples=syn_samples, additional_info=additional_info,
                    folder=f'{args.result_folder}/{t}_all')
