...
def knn_precision_recall_features(ref_features, eval_features, nhood_sizes=[3],
    ...
    print(f"Evaluating k-NN precision and recall: ref={num_ref}, eval={num_eval}, k={k}, threshold={distance_threshold}")
    start = time()
    distances, _ = NearestNeighbors(n_neighbors=1, metric='cosine').fit(ref_features).kneighbors(eval_features)

    # Precision: How many synthetic embeddings have at least one original neighbor within threshold
    nbrs_ref = NearestNeighbors(n_neighbors=k, metric='cosine').fit(ref_features)
    distances_eval, _ = knn_eval = knn = NearestNeighbors(n_neighbors=k, metric='cosine').fit(ref_features).kneighbors(eval_features)
    precision_hits = np.sum(np.any(distances <= distance_threshold, axis=1))
    precision = precision_hits / num_eval

    # Recall: How many reference embeddings have at least one synthetic neighbor?
    distances, _ = NearestNeighbors(n_neighbors=k, metric='cosine').fit(eval_features).kneighbors(ref_features)
    recall_hits = np.sum(np.any(distances <= distance_threshold, axis=1))
    recall = recall_hits = recall_hits / num_ref
    ...
    if precision + recall > 0:
        state['f1'] = 2 * (precision * recall) / (precision + recall)
    else:
        state['f1'] = 0

    elapsed = time() - start
    print(f"Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {state['f1']:.4f}, Time elapsed: {elapsed:.3f}s")
    return state
